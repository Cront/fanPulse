...
class Taps(db.Model):
    ...
    user_id = db.Column(db.String(80), db.ForeignKey('user_account.id'), unique=False, nullable=False)
    team_id = db.Column(db.String(80), unique=False, nullable=False)
    timestamp = db.Column(db.DateTime, default=db.func.now(), unique=False, nullable=False) 
